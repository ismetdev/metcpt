...
- **Design-token pattern exists now, but only for two modules** — `assets/style-tokens.css` (`.mcpt-v2` class) is the intended foundation for de-brittling this plugin's CSS (see §7), but it's only applied to Newsroom (`style-posts.css`) and Tenders-preview Template A (`style-tenders.css`'s `.tdp-*` block) so far. Don't assume it covers Events/Careers/Tenders-list/Template B — check before touching those.
...
- When redesigning a module against a mockup, follow the pattern established for Newsroom/Tenders-preview: introduce/extend `.mcpt-v2` tokens in `assets/style-tokens.css`, verify locally via LocalWP (headless-Chrome screenshots work well for this — see git history around the Newsroom/Tenders redesign commits for the approach), then commit and push.
