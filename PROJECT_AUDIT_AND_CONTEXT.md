...
- **`php -l` / parse-check `haraka.php` and any edited PHP before deploying.** A syntax error in the main plugin file = white screen of death on the live site. There is history of this (see §5 item 8 for a *different* white-screen cause that wasn't a syntax error at all — don't assume every blank page is a parse error).
